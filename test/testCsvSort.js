...
describe('CSV sort function', function () {
    let csv;
    ...
    it('should sort CSV by ascending order when column name is provided', function (done) {
        const sortOpts = {
            hasHeader: true,
            sortByColumn: 'Address Number',
            orderBy: 'ASC'
        }
        csvSort(csv, sortOpts, function (err, sortedCsv) {
            assert.ifError(err);
    ...
    it('should sort CSV based on critria when column number string is provided', function (done) {
        const sortOpts = {
            hasHeader: true,
            sortByColumn: '3',
            orderBy: 'ASC'
    ...
    it('should sort CSV based on critria when column number integer is provided', function (done) {
        const sortOpts = {
            hasHeader: true,
            sortByColumn: 2,
            orderBy: 'ASC'
    ...
    it('should sort CSV with no header row', function (done) {
        csv = csv.split('\n');
        csv.shift()
        csv = csv.join('\n');
        const sortOpts = {
    ...
    it('should default to ascending order if no sort order is provided', function (done) {
        const sortOpts = {
            hasHeader: true,
            sortByColumn: '4',
            orderBy: null
    ...
    it('should sort by ascending order if ASC is provided', function (done) {
        const sortOpts = {
            hasHeader: true,
            sortByColumn: '5',
            orderBy: 'ASC'
    ...
    it('should sort by descending order if DESC is provided', function (done) {
        const sortOpts = {
            hasHeader: true,
            sortByColumn: '5',
            orderBy: 'DESC'
    ...
    it('should sort by descending order without case sensitivity', function (done) {
        const sortOpts = {
            hasHeader: true,
            sortByColumn: '5',
            orderBy: 'desc'
    ...
    it('should return error if no CSV string is provided', function (done) {
        ...
        csvSort(null, sortOpts, function (err, sortedCsv) {
    ...
    it('should return error if invalid CSV string is provided', function (done) {
        ...
        csvSort([1,2,3], sortOpts, function (err, sortedCsv) {
            expect(sortedCsv).to.not.be.ok;
            expect(err.message).to.equal('Unable to parse CSV string. Make sure that a valid CSV string has been provided.')
            done();
        });
